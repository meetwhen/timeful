...
Shared visual states had drifted across component-local raw palette values and framework-specific overrides.

## Decision

The frontend uses app-level semantic styling tokens as its shared visual-state contract:
...
Shared visual states can be reused across feature views and library-rendered surfaces without making raw palette choices part of component contracts.
