...
Graphify produces derived output under `graphify-out/`. The repository tracks
...
signatures (`.graphify_labels.json.sig`). Graphify regenerates these files after
commits, and versioning them creates distracting worktree churn without
preserving source material.

Do not track syntactic output. It is quick to index locally, so committing it
