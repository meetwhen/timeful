...
- Treat backlog tasks as source material, not as requirement records. Extract
...
- Recover applicability context from the task, linked artifacts, and current
  behavior. Do not generalize a requirement beyond the page, event kind, mode,
  role, or state the source establishes.
- Preserve the requirement's permanent ID and ID-only filename. Use the YAML
