...
defines the term's complete behavior, constraints, and exceptions.
The
...
A glossary entry may name approved aliases or rejected
...
in every paragraph, list item, and table cell to its `glossary.md` anchor.
Link
...
already linked.
An author may omit a repeated link when it would make a dense
terminology table harder to read.

The rule makes the definition available where a reader needs it without
requiring a link on every repeated use.
Documentation checks may verify that
