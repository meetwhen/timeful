...
async function openEditDialog(page: Page) {
  ...
  const advancedOptionsButton = card.getByRole("button", {
...
async function main() {
  ...
  try {
    ...
    const advanced = editorCard.getByRole("button", {
      name: /advanced options/i,
    })
    ...
    const vueApp = (
      document.querySelector("#app") as { __vue_app__?: unknown } | null
    )?.__vue_app__
    ...
    const allDaysDebug = await page.evaluate(() => {
      ...
      const allScheduleOverlaps = document.querySelectorAll(
        ".schedule-overlap-layout",
      )
      if (allScheduleOverlaps.length === 0)
        return { error: "no schedule overlap" }

      // Access the Vue component instance through internal property
      const scheduleOverlapEl =
        allScheduleOverlaps[0] as VueParentComponentElement
      ...
      const allDaysArr = ss.allDays as
        | { value: Record<string, unknown>[] }
        | undefined
      const daysArr = ss.days as
        | { value: Record<string, unknown>[] }
        | undefined
      ...
      return {
        allDays: allDaysArr?.value.map((d) => ({
          ...
          dateObjectISO: (
            d.dateObject as { toString: () => string }
          ).toString(),
        })),
