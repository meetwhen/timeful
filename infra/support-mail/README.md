...
- `stalwart` container publishes ports **25** (inbound SMTP) and **993**
  (IMAPS) directly.
  IMAP clients connect to `mail.timeful.fun:993`.
- The container HTTP listener (`8080`) is bound to `127.0.0.1` on the VM
  only.
  Caddy fronts `https://mail.timeful.fun` (admin UI, JMAP, autoconfig).
...
- TLS certificates come from Caddy's Let's Encrypt issuance for
  `mail.timeful.fun` (Caddy owns ACME on 443).
  The certificate and key are
...
- VM with Docker, a static public IP, and TCP **25**, **993**, **443**, **80**
  ...
  Some providers block inbound 25 by default
...
3. Directory: _Internal Directory_.
