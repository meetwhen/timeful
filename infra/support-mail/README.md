...
Receive-only mail server for the timeful.fun domain. Sending stays with the
...
- `stalwart` container publishes ports **25** (inbound SMTP) and **993**
  (IMAPS) directly. IMAP clients connect to `mail.timeful.fun:993`.
- The container HTTP listener (`8080`) is bound to `127.0.0.1` on the VM
  only. Caddy fronts `https://mail.timeful.fun` (admin UI, JMAP, autoconfig).
...
- TLS certificates come from Caddy's Let's Encrypt issuance for
  `mail.timeful.fun` (Caddy owns ACME on 443). The certificate and key are
...
- Caddy running on the VM so `https://mail.timeful.fun` terminates there.
  This repo's Caddy auto-imports the new site file via `import sites/*.caddy`.

## 1. DNS (manual, at the timeful.fun provider)

| Type | Name | Value | Purpose |
|---|---|---|---|
| A | `mail.timeful.fun` | VM public IP | server identity |
| MX | `timeful.fun` | `10 mail.timeful.fun.` | receive mail |
| PTR | reverse zone (VM provider) | `mail.timeful.fun` | inbound credibility |
| CAA | `timeful.fun` | `0 issue "letsencrypt.org"` | allow Let's Encrypt certs |
| SPF | `timeful.fun` | leave as-is (relay's infra) | do NOT list the VM |
| DMARC | `_dmarc.timeful.fun` | keep current policy; tighten to `p=reject` only after relay DKIM/SPF alignment is verified | |
| MTA-STS (optional) | `_mta-sts.timeful.fun` + `mta-sts.timeful.fun` CNAME | see Stalwart docs | only add once TLS works |

Wait for propagation (`dig MX timeful.fun`, `dig A mail.timeful.fun`).
...
network). Reload Caddy and verify `https://mail.timeful.fun` serves the
...
`postmaster@timeful.fun` alias. IMAP client settings:

- host `mail.timeful.fun`, port **993** (SSL/TLS)
- login `support@timeful.fun`
...
- `dig MX timeful.fun` -> mail host; `dig A mail.timeful.fun` -> VM IP
- external: `swaks --to support@timeful.fun --from someone@example.org
  --server mail.timeful.fun` (or send from any webmail provider)
- check `docker compose logs stalwart` for delivery, then read the message
  via IMAP
- admin UI and discovery: `curl https://mail.timeful.fun/.well-known/jmap`
...
- Self-served mail to `support@timeful.fun` from the listmon relay is
