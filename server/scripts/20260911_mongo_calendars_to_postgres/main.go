// mongo_calendars_to_postgres backfills authoritative PostgreSQL calendar
// integrations from retained MongoDB user documents. It never modifies the
// MongoDB source. Every migration unit is one owner account and is written in
// one PostgreSQL transaction that also records completion in the migration
// ledger, so a rerun resumes after the first incomplete unit and repeated runs
// never create duplicate connections, credentials, sub-calendars, or
// preferences.
package main
...
const usage = "usage: go run ./scripts/20260911_mongo_calendars_to_postgres [--apply] [--batch-size N] [--limit N] [--batch-label label]"
...
// migrationSummary reports the work observed in one run. Migrated counts owner
// accounts whose writes committed in this run; Skipped counts units already
// completed by an earlier run; Quarantined counts retained units that could not
// be migrated because their owner or credential was unusable.
...
func main() {
	...
	summary, complete, err := migrate.migrateCalendars(ctx, config)
	if err != nil {
		fatal(fmt.Errorf("migrate calendars: %w", err))
	}
	fmt.Printf("calendars: scanned=%d migrated=%d skipped=%d quarantined=%d\n", summary.Scanned, summary.Migrated, summary.Skipped, summary.Quarantined)
	if !complete {
	...
	if !config.apply {
		fmt.Println("preflight passed; rerun with --apply to write PostgreSQL calendar rows")
		return
	}
...
func parseConfiguration(arguments []string) (configuration, error) {
	flags := flag.NewFlagSet("mongo_calendars_to_postgres", flag.ContinueOnError)
	flags.SetOutput(os.Stderr)
	config := configuration{}
	flags.BoolVar(&config.apply, "apply", false, "write PostgreSQL calendar rows")
...
// recordQuarantine records an ambiguous or unmigratable retained unit. The
// ledger is append-only, so a replay preserves the same decision, and it is a
// no-op during a read-only preflight.
